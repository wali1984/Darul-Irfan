...
This project is authored from Windows and compiled and tested by Codemagic on
macOS. The project file is generated from `project.yml` with XcodeGen on a Mac.
...
The Swift uses Swift 5.9 and iOS 17 APIs without experimental features. The
Codemagic verification workflow is the authoritative compile and test gate. A
sanity checklist:
...
- **Typical CI issues** to watch for: `@Observable` observation scoping,
  `Sendable` warnings on delegate bridges, and asset catalog
...
- **Swift unit and UI tests run in Codemagic's `ios-verify` workflow.** The
  v1.8.0 verification completed successfully. `DarulIrfanTests/` covers prayer calculations against
  ...
  defensively and asserts the tab bar.
...
| Widgets: configurable devotional dashboard, Next Prayer and Zikr (Home + Lock Screen), Today's Times, Smart Stack relevance, branded Live Activity, App Group snapshot bridge | Implemented |
...
| Swift unit/UI test suites | Implemented and run by Codemagic `ios-verify` — see Testing |
| Live Activities (next prayer) | Implemented with explicit stale-state recovery; opt-in/device verification required |
| Apple Watch azan/zikr app + prayer/zikr complications and local devotional progress | Implemented; signing/device verification required |
