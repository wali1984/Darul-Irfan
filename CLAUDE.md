...
- Content rights: permission from the site owner is **not yet confirmed** — seed data ships metadata + source links (rightsStatus: `linkOnly`) plus a small number of clearly-attributed public excerpts. The ingest pipeline in `Tools/ContentIngest` has a `--full-text` flag to be used only after permission is confirmed.
